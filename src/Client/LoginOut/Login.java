		...
		try{
			...
			if(rs.next()){	//if an entry is present in the resultset
                if (rs.getString(2).equals(pwd)) {	//if the password in the entry is the same as the one entered by user
                	if(rs.getString(3).equals("T")){	//if the column 3 return "T", user has been blocked by admin
                		...
                		if(rs.getString(5).equals("F")){	//if the column 4 return "F", user has not authenticated email ID

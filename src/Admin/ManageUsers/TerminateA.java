...
/**
 * Servlet implementation class AdminT
 */
@WebServlet("/TerminateA")
public class TerminateA extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("admin")==null){	//if admin session does not exist
			...
			try{
				...
				int i=st.executeUpdate("delete from admins where email = '"+email+"'");	//int i is used to execute statement, deleting the records of the selected admin from the DB
				if(i>0)	//int i will be greater then 0 if the update to the DB was successful
				{
					out.println(" admin terminated.");
				}
			}
			catch(Exception se)
			{
